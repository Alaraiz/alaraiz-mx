...
export default function LandingClient({
  ...
  useEffect(() => {
    if (initialFacilitators.length > 0) return;
      ...
      <div className="discover-panel">
        {active === "faq" && (
          <div className="discover-panel-inner">
            {faqItems.map((item) => (
              <div key={item.n} className={`faq-item-block${openFaq === item.n ? " is-open" : ""}`}>
                <button
                  type="button"
                  className="faq-item-q"
                  onClick={() => setOpenFaq(openFaq === item.n ? "" : item.n)}
                  aria-expanded={openFaq === item.n}
                >
                  ...
                  <span className="faq-toggle" aria-hidden="true">{openFaq === item.n ? "-" : "+"}</span>
                </button>
                {openFaq === item.n && (
